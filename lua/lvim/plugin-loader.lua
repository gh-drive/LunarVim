  ...
  if not utils.is_directory(lazy_install_dir) then
    ...
    if utils.is_directory(core_plugins_dir) then
      ...
      vim.fn.system({
        ...
        "clone",
        "--branch=stable",
        "https://e.coding.net/zydou/vim/lazy.nvim.git",
        lazy_install_dir,
      })
...
function plugin_loader.reload(spec)
  ...
  local not_installed_plugins = vim.tbl_filter(function(plugin) return not plugin._.installed end, Config.plugins)

  require("lazy.manage").clear()

  if #not_installed_plugins > 0 then lazy.install({ wait = true }) end
...
function plugin_loader.get_core_plugins()
  ...
  for _, spec in pairs(plugins) do
    if spec.enabled == true or spec.enabled == nil then table.insert(names, get_name(spec[1])) end
  end
  return names
end
